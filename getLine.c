...
#if USE_GETLINE
		rn = getline(buf, &len_pt, stdin);
#else
		rn = _getline(info, buf, &len_pt);
#endif
		if (rn > 0)
		{
			if ((*buf)[rn - 1] == '\n')
			{
				(*buf)[rn - 1] = '\0'; /* remove trailing newline */
				rn--;
			}
...
ssize_t get_input(info_t *info)
{
	...
	if (len)	/* we have commands left in the chain buffer */
	{
		...
		*buf_p = pt; /* pass back pointer to current command position */
